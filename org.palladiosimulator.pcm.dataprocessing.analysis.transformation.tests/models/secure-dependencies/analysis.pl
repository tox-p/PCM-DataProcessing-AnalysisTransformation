...
secureDependencies(OP, S, R, V) :- T = 'EnumCharacteristicType Critical (_5KwWYP7hEeizQdLqRSCK5g)', isOperation(OP), hasProperty(OP, T), S=[OP|_], stackValid(S), operationReturnValue(OP, R), secureDependenciesViolation(OP, T, S, R, V).
